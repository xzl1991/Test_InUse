...
public class LinkedListsCircle<T>{
	/***
	...
	private int size;
	//创建空链表
	public LinkedListsCircle(){
		head = null;
		tail = null;
	}
	
	//以指定元素创建链表，该链表只有一个元素
	public LinkedListsCircle(T element){
		head = new Node(element,tail);//尾节点
	...
	public  void add(T element){
		...
		if(head==null){
			...
			//不是空创建新节点,包括 tail 和  data 元素
			Node newNode = new Node(element,tail);
			//尾节点的next 指向 新节点，当前的 tail
			tail.next = newNode;
			//新节点作为 新的尾节点
			tail = newNode;
			tail.next=head;//尾节点指向 head
		}
		size++;
	} 
	//向链式线性表指定位置添加元素
